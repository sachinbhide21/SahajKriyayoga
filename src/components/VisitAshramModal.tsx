...
export const VisitAshramModal: React.FC<VisitAshramModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-white/80 backdrop-blur-md animate-fade-in">
      <div className="bg-white border border-amber-200 rounded-3xl max-w-xl w-full p-6 sm:p-8 shadow-2xl relative overflow-hidden max-h-[90vh] overflow-y-auto">
        
        {/* Close Button */}
        <button
          onClick={onClose}
          className="absolute top-5 right-5 p-2 rounded-full bg-slate-100 text-slate-500 hover:text-slate-900 :text-amber-700 transition-colors"
        >
          <X className="w-5 h-5" />
        </button>
        ...
        {submitted ? (
          <div className="text-center py-8 space-y-4">
            ...
            <div className="p-4 rounded-2xl bg-amber-50 border border-amber-200 text-xs text-slate-700 text-left space-y-1">
              <p className="font-bold text-amber-900 ">Direct Contact Details:</p>
          ...
          <div className="space-y-6">
            ...
            <div>
              <div className="inline-flex items-center space-x-1.5 px-3 py-1 rounded-full bg-amber-100 text-amber-900 text-xs font-bold uppercase tracking-wider mb-2 border border-amber-300/50">
              ...
              <p className="text-xs text-slate-600 mt-1">
                Selected Centre: <strong className="text-amber-800 ">{selectedCentre.name} ({selectedCentre.branchName || selectedCentre.state})</strong>
              </p>
            </div>
            ...
            <div className="p-4 rounded-2xl bg-amber-50/80 border border-amber-200 flex items-start space-x-3">
              ...
              <div className="text-xs space-y-1">
                <p className="font-bold text-slate-900 ">{selectedCentre.location}</p>
                <p className="text-slate-600 ">{selectedCentre.description}</p>
              </div>
            </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    ...
                    className="w-full px-3.5 py-2 rounded-xl text-xs bg-slate-50 border border-slate-300 text-slate-900 focus:ring-2 focus:ring-amber-500 outline-none"
                  />
                </div>
              ...
              <div>
                ...
                <textarea
                  ...
                  className="w-full px-3.5 py-2 rounded-xl text-xs bg-slate-50 border border-slate-300 text-slate-900 focus:ring-2 focus:ring-amber-500 outline-none resize-none"
                />
              </div>
              ...
              <div className="pt-2 flex items-center justify-end space-x-3">
                <button
                  type="button"
                  onClick={onClose}
                  className="px-5 py-2 rounded-full text-xs font-bold text-slate-600 hover:bg-slate-100 :bg-slate-800 transition-colors"
                >
                  Cancel
                </button>
                <button
