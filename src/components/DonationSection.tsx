...
export const DonationSection: React.FC = () => {
  ...
  return (
    <section id="donate" className="py-20 bg-gradient-to-b from-amber-950 via-slate-950 to-amber-950 text-white relative overflow-hidden">
                ...
                <div
                  ...
                  className={`p-4 rounded-2xl cursor-pointer transition-all border ${
                    selectedSeva === cause.name
                      ...
                      : 'bg-slate-900/60 border-amber-500/20 text-amber-200/80 hover:bg-slate-800'
                  }`}
                >
                  <div className="flex items-center justify-between">
                    <h4 className="font-serif font-bold text-sm text-amber-200">{cause.name}</h4>
                    <span className="text-amber-400 font-bold">{selectedSeva === cause.name ? '✓' : ''}</span>
                  </div>
                  <p className="text-xs text-amber-200/70 mt-1">{cause.desc}</p>
                </div>
              ))}
            </div>

            {/* Tax Exemption Badge */}
            <div className="p-4 rounded-2xl bg-amber-900/40 border border-amber-500/30 flex items-center space-x-3 text-xs text-amber-200">
              <ShieldCheck className="w-6 h-6 text-emerald-400 shrink-0" />
              <div>
                <p className="font-bold text-amber-300">80G Income Tax Benefit</p>
                <p className="text-[11px] text-amber-200/80">Donations qualify for 80G tax deduction. Official receipts issued instantly.</p>
              </div>
            </div>
          </div>

          {/* Bank & UPI Details Right */}
          <div className="lg:col-span-6 bg-slate-900/90 rounded-3xl p-6 sm:p-8 border border-amber-500/30 shadow-2xl space-y-6">
            
            <div className="text-center space-y-2 pb-4 border-b border-amber-500/20">
              <h4 className="font-serif font-bold text-xl text-amber-200">Direct Bank & UPI Transfer</h4>
              <p className="text-xs text-amber-200/70">Scan QR Code or transfer directly to Ashram Bank Account</p>
            </div>
            ...
            <div className="flex flex-col sm:flex-row items-center justify-center gap-6">
              ...
              <div className="space-y-2 text-xs text-amber-200 text-center sm:text-left">
                <p className="font-bold text-amber-300 text-sm">Brahmarishi Satyananda Sannyas Ashram</p>
            ...
            <p className="text-[11px] text-amber-200/60 text-center">
              After transfer, please share transaction reference via WhatsApp at <strong className="text-amber-300">+91 99094 78275 / +91 90094 11592</strong> to receive 80G receipt.
            </p>
