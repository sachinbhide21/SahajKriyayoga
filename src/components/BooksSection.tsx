...
export const BooksSection: React.FC = () => {
  ...
  return (
    <section id="publications" className="py-20 bg-[#FAF8F5] dark:bg-slate-900 transition-colors duration-300">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-12">
        
        {/* Title Header */}
        <div className="text-center max-w-3xl mx-auto space-y-3">
          <div className="inline-flex items-center space-x-2 px-3 py-1 rounded-full bg-amber-100 dark:bg-amber-900/40 text-amber-900 dark:text-amber-300 text-xs font-bold uppercase tracking-wider border border-amber-200 dark:border-amber-800">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
          {BOOK_PUBLICATIONS.map((book) => (
            <div
              key={book.id}
              className="bg-white dark:bg-slate-800 rounded-3xl p-5 shadow-lg border border-amber-200/60 dark:border-slate-700 hover:border-amber-500 transition-all duration-300 flex flex-col justify-between group"
            >
              <div className="space-y-4">
                {/* Book Cover Frame - Title Only SVG Cover */}
                <div className="h-64 rounded-2xl overflow-hidden bg-amber-950/20 relative shadow-md group-hover:shadow-xl transition-shadow">
                  <img
                    src={book.coverImage}
                    alt={book.title}
                ...
                <div className="space-y-1">
                  <h3 className="font-serif font-bold text-base text-slate-900 dark:text-amber-100 line-clamp-2">
                    {book.title}
                  </h3>
              ...
              {/* Action Buttons */}
              <div className="mt-5 pt-3 border-t border-slate-100 dark:border-slate-700 space-y-2">
                <a
                ...
                <button
                  ...
                  className="w-full py-2 rounded-xl text-xs font-semibold text-slate-700 dark:text-slate-300 bg-slate-100 dark:bg-slate-700/60 hover:bg-slate-200 dark:hover:bg-slate-700 transition-colors flex items-center justify-center space-x-1.5"
                >
      ...
      {selectedBook && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-md animate-fade-in">
          <div className="bg-white dark:bg-slate-900 rounded-3xl max-w-lg w-full p-6 sm:p-8 border border-amber-300 dark:border-slate-700 shadow-2xl relative space-y-4">
            
            <button
              onClick={() => { setSelectedBook(null); setRequestSent(false); }}
              className="absolute top-4 right-4 p-2 rounded-full bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 hover:bg-red-500 hover:text-white transition-colors"
            >
              <X className="w-5 h-5" />
            </button>
            ...
            {!requestSent ? (
              <>
                <div className="flex items-center space-x-3">
                  <div className="w-12 h-12 rounded-2xl bg-amber-100 dark:bg-amber-900/40 flex items-center justify-center text-amber-800 dark:text-amber-300 font-bold text-xl">
                    📖
                  </div>
                  <div>
                    <h3 className="font-serif font-bold text-lg text-slate-900 dark:text-amber-100">
                      Order Book Publication
                    </h3>
                    <p className="text-xs text-amber-700 dark:text-amber-400 font-semibold">{selectedBook.title}</p>
                ...
                <div className="bg-emerald-50 dark:bg-emerald-950/40 border border-emerald-200 dark:border-emerald-800/60 rounded-2xl p-3.5 flex items-center justify-between gap-3">
                  <div className="space-y-0.5">
                    <p className="text-xs font-bold text-emerald-900 dark:text-emerald-300">Fastest Way to Order</p>
                    <p className="text-[11px] text-emerald-700 dark:text-emerald-400">Chat directly on WhatsApp to check availability and dispatch</p>
                  </div>
                  <a
                ...
                >
                  <div>
                    ...
                    <input
                      ...
                      className="w-full px-3 py-2 rounded-xl border border-amber-300 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-900 dark:text-white"
                    />
                  </div>
                  ...
                  <div>
                    <label className="block text-slate-700 dark:text-slate-300 font-bold mb-1">Postal Delivery Address *</label>
                    <textarea
                      required
                      rows={2}
              ...
              <div className="text-center py-6 space-y-4">
                ...
                <h3 className="font-serif font-bold text-xl text-slate-900 dark:text-amber-100">
