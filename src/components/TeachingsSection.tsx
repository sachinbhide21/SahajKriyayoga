...
export const TeachingsSection: React.FC = () => {
  ...
  return (
    <section id="teachings" className="py-20 bg-[#F5F0EB] dark:bg-slate-950 transition-colors duration-300">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {TEACHINGS.map((item) => (
            <div
              ...
              className="group bg-white dark:bg-slate-900 rounded-3xl p-7 shadow-lg border border-amber-200/60 dark:border-slate-800 hover:border-amber-400 dark:hover:border-amber-500 hover:shadow-2xl transition-all duration-300 transform hover:-translate-y-2 cursor-pointer flex flex-col justify-between"
            >
              <div className="space-y-4">
                {/* Header Icon + Titles */}
                <div className="flex items-center justify-between">
                  <div className="w-12 h-12 rounded-2xl bg-amber-100 dark:bg-amber-900/40 flex items-center justify-center group-hover:scale-110 transition-transform">
                    {getIcon(item.iconName)}
                  </div>
                  {item.hindiTitle && (
                    <span className="text-xs font-serif font-bold text-amber-800 dark:text-amber-300 bg-amber-50 dark:bg-slate-800 px-3 py-1 rounded-full border border-amber-200/50">
                      {item.hindiTitle}
                    </span>
                  )}
                </div>
                ...
                <div className="space-y-1">
                  <h3 className="text-xl font-serif font-bold text-slate-900 dark:text-amber-100 group-hover:text-amber-600 dark:group-hover:text-amber-400 transition-colors">
                    {item.title}
                  </h3>
              ...
              {/* Card Footer Button */}
              <div className="mt-6 pt-4 border-t border-slate-100 dark:border-slate-800 flex items-center justify-between text-xs font-bold text-amber-700 dark:text-amber-400 group-hover:text-amber-900 dark:group-hover:text-amber-300">
      ...
      {selectedTeaching && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-md animate-fade-in">
          <div className="bg-white dark:bg-slate-900 rounded-3xl max-w-2xl w-full p-6 sm:p-8 border border-amber-300 dark:border-slate-700 shadow-2xl relative max-h-[90vh] overflow-y-auto space-y-6">
            ...
            <div className="space-y-3 bg-amber-50 dark:bg-slate-800/80 p-5 rounded-2xl border border-amber-200/60 dark:border-slate-700">
              <h4 className="font-serif font-bold text-amber-900 dark:text-amber-300 text-sm">
            ...
            <div className="p-4 rounded-xl bg-slate-100 dark:bg-slate-800 text-xs text-slate-600 dark:text-slate-400">
              <p className="font-bold text-amber-800 dark:text-amber-300 mb-1">Practice Overview:</p>
