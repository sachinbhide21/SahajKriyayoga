...
export const YouTubeSection: React.FC = () => {
  ...
  return (
    <section id="youtube" className="py-20 bg-[#F5F0EB] dark:bg-slate-950 transition-colors duration-300">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-16">
        
        {/* Section Header */}
        <div className="text-center max-w-3xl mx-auto space-y-3">
          <div className="inline-flex items-center space-x-2 px-3 py-1 rounded-full bg-red-100 dark:bg-red-950/50 text-red-700 dark:text-red-400 text-xs font-bold uppercase tracking-wider border border-red-200/50 dark:border-red-900/50">
        ...
        <div className="bg-gradient-to-r from-red-950 via-slate-900 to-amber-950 rounded-3xl p-6 sm:p-8 text-white shadow-2xl border border-red-500/30 relative overflow-hidden">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
              {YOUTUBE_CHANNEL_INFO.latestVideos.map((video) => (
                <div
                  ...
                  className="group bg-slate-900/80 hover:bg-slate-900 border border-white/10 hover:border-red-500/60 rounded-2xl overflow-hidden shadow-lg transition-all duration-300 cursor-pointer flex flex-col justify-between"
                >
                  <div className="relative aspect-video bg-black overflow-hidden">
                    ...
                    <span className="absolute bottom-2 right-2 px-1.5 py-0.5 rounded bg-black/80 text-[10px] font-mono text-amber-200">
                      {video.duration}
                    </span>
                  </div>
                  ...
                  <div className="p-3.5 space-y-1.5">
                    ...
                    <h5 className="font-serif font-bold text-xs text-amber-100 group-hover:text-red-300 transition-colors line-clamp-2 leading-snug">
                      {video.title}
                    </h5>
        ...
        <div className="space-y-6">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-amber-200/60 dark:border-slate-800 pb-3">
            <div>
              <h3 className="text-2xl font-serif font-extrabold text-slate-900 dark:text-amber-100 flex items-center gap-2">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            {YOUTUBE_PLAYLISTS.map((pl) => (
              <div
                key={pl.id}
                className="bg-white dark:bg-slate-900 rounded-3xl overflow-hidden shadow-xl border border-amber-200/80 dark:border-slate-800 hover:border-red-500/50 transition-all duration-300 flex flex-col justify-between"
              >
                <div>
                  ...
                  <div className="relative h-48 bg-slate-950 overflow-hidden group">
                    ...
                    <div className="absolute bottom-4 left-4 right-4 flex items-end justify-between">
                      <div>
                        ...
                        <h4 className="font-serif font-extrabold text-lg sm:text-xl text-amber-100 mt-1 drop-shadow">
                          {pl.title}
                        </h4>
                      </div>
                      ...
                      <div className="grid grid-cols-1 gap-1.5">
                        {pl.featuredVideoIds.slice(0, 3).map((vidId, idx) => (
                          <div
                            ...
                            className="flex items-center gap-2 p-2 rounded-xl bg-slate-50 dark:bg-slate-800/60 hover:bg-red-50 dark:hover:bg-slate-800 cursor-pointer text-xs transition-colors group"
                          >
                            <span className="w-5 h-5 rounded-full bg-red-100 dark:bg-red-950 text-red-700 dark:text-red-400 flex items-center justify-center text-[10px] font-bold group-hover:bg-red-600 group-hover:text-white transition-colors">
                              {idx + 1}
                            </span>
                ...
                <div className="p-6 pt-0 flex items-center justify-between gap-3">
                  ...
                  <a
                    ...
                    target="_blank"
                    rel="noopener noreferrer"
                    className="py-2.5 px-4 rounded-xl bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-800 dark:text-slate-200 font-bold text-xs flex items-center gap-1.5 transition-colors"
                  >
        ...
        <div id="audio-chanting" className="bg-gradient-to-br from-amber-950 via-slate-900 to-amber-900 rounded-3xl p-6 sm:p-8 text-white shadow-2xl border border-amber-500/30 space-y-6">
              ...
              return (
                <div
                  ...
                  className={`p-5 rounded-2xl border transition-all duration-300 flex flex-col justify-between space-y-4 ${
                    isCurrentPlaying
                      ? 'bg-amber-950/90 border-amber-400 shadow-xl shadow-amber-500/20 scale-[1.02]'
                      : 'bg-slate-900/80 border-amber-500/20 hover:border-amber-400/60 hover:bg-slate-900'
                  }`}
                >
                  <div className="space-y-3">
                    <div className="flex items-center justify-between">
                      <div className={`w-10 h-10 rounded-xl flex items-center justify-center shrink-0 ${
                        isCurrentPlaying ? 'bg-amber-500 text-slate-950 animate-pulse' : 'bg-amber-500/20 text-amber-300'
                      }`}>
                    ...
                    <div className="space-y-1">
                      <h4 className="font-serif font-bold text-base text-amber-100">{track.title}</h4>
                      <p className="text-xs font-semibold text-amber-300/90">{track.sanskritTitle}</p>
                    </div>
                  </div>
                  ...
                  <button
                    ...
                    className={`w-full py-2.5 px-4 rounded-xl text-xs font-bold flex items-center justify-center gap-2 transition-all ${
                      isCurrentPlaying
                        ...
                        : 'bg-amber-900/50 hover:bg-amber-600 text-amber-100 border border-amber-500/40'
                    }`}
                  >
                    {isCurrentPlaying ? (
                      <>
        ...
        <div className="space-y-6 pt-6">
          <div className="text-center space-y-2">
            <h3 className="text-2xl font-serif font-extrabold text-slate-900 dark:text-amber-100">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {filteredDiscourses.map((disc) => (
              <div
                ...
                className="group bg-white dark:bg-slate-900 rounded-3xl overflow-hidden shadow-lg border border-amber-200/60 dark:border-slate-800 hover:border-red-500 hover:shadow-2xl transition-all duration-300 transform hover:-translate-y-1.5 cursor-pointer flex flex-col justify-between"
              >
                <div>
                  ...
                  <div className="p-5 space-y-2">
                    <h4 className="font-serif font-bold text-lg text-slate-900 dark:text-amber-100 group-hover:text-red-600 dark:group-hover:text-red-400 transition-colors line-clamp-2">
                      {disc.title}
                    </h4>
                ...
                <div className="p-5 pt-0 flex items-center justify-between text-xs text-slate-500 dark:text-slate-400 border-t border-slate-100 dark:border-slate-800 mt-2">
                  ...
                  <span className="font-bold text-red-600 dark:text-red-400">Watch Discourse →</span>
                </div>
              </div>
            ))}
          </div>
      ...
      {selectedVideo && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/90 backdrop-blur-md animate-fade-in">
          <div className="bg-slate-900 rounded-3xl max-w-4xl w-full p-6 border border-amber-500/30 shadow-2xl relative space-y-4">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 text-xs text-slate-400 pt-2 border-t border-slate-800">
              <div className="space-y-0.5">
                <span className="text-amber-300 font-semibold">{selectedVideo.speaker}</span>
              ...
              <div className="flex items-center gap-3">
                <a
                  ...
                  target="_blank"
                  rel="noopener noreferrer"
                  className="px-4 py-2 rounded-full text-xs font-bold text-amber-200 bg-slate-800 hover:bg-slate-700 flex items-center gap-1.5 transition-colors"
                >
