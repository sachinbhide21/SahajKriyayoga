...
export const GuruDetailModal: React.FC = () => {
  ...
  return (
    <div className="fixed inset-0 z-50 overflow-y-auto bg-amber-50/95 backdrop-blur-md animate-fade-in transition-all min-h-screen">
      
      {/* Top Floating Control Bar */}
      <div className="sticky top-0 z-40 bg-white/95 backdrop-blur-md border-b border-amber-200/80 shadow-sm px-4 sm:px-8 py-3 flex items-center justify-between">
        
        {/* Breadcrumb Navigation */}
        <div className="flex items-center gap-2 text-xs sm:text-sm font-medium text-slate-600 overflow-x-auto whitespace-nowrap">
          <button 
            onClick={() => setSelectedGuruForBio(null)} 
            className="hover:text-amber-700 :text-amber-400 flex items-center gap-1 font-bold text-amber-800 "
          >
            <Home className="w-3.5 h-3.5" />
            <span>{isHi ? "मुख्य पृष्ठ" : "Home"}</span>
          </button>
          <ChevronRight className="w-3.5 h-3.5 text-amber-500 shrink-0" />
          <span className="text-slate-500 font-semibold">{isHi ? "गुरु परंपरा" : "Guru Parampara"}</span>
          <ChevronRight className="w-3.5 h-3.5 text-amber-500 shrink-0" />
          <span className="text-amber-900 font-bold truncate max-w-[150px] sm:max-w-none">
        ...
        <div className="flex items-center gap-2 shrink-0">
          ...
          <div className="inline-flex rounded-lg p-0.5 bg-slate-100 border border-slate-200 ">
            <button
              ...
              className={`px-2.5 py-1 text-xs font-bold rounded-md transition-all ${
                isHi ? 'bg-amber-700 text-white shadow-sm' : 'text-slate-600 hover:text-amber-700'
              }`}
            >
              हिन्दी
            </button>
          ...
          <button
            ...
            className="hidden md:flex px-2.5 py-1 text-xs font-bold rounded-lg bg-amber-100/70 border border-amber-300 text-amber-900 hover:bg-amber-200/80 items-center gap-1"
          ...
          <button
            ...
            className="p-1.5 rounded-lg bg-slate-100 text-slate-700 hover:bg-amber-100 flex items-center gap-1 text-xs font-semibold"
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-10">
        ...
        {/* SEO Meta Box */}
        {showSeoMeta && (
          <div className="p-4 sm:p-5 rounded-2xl bg-amber-100/90 border border-amber-300 text-xs space-y-2 animate-fade-in">
            <p className="font-bold text-amber-950 flex items-center gap-1.5 uppercase tracking-wider">
              <Tag className="w-4 h-4 text-amber-600" /> SEO & Web Publishing Metadata Specs
            </p>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-slate-800 font-mono text-[11px] leading-relaxed">
              <div><span className="font-bold text-amber-800 ">SEO Title:</span> {g.seoMetaTitle || `${g.name} Official Biography - Kriya Yoga Master`}</div>
              <div><span className="font-bold text-amber-800 ">URL Slug:</span> {g.urlSlug || `/guru-parampara/${g.id}`}</div>
              <div className="sm:col-span-2"><span className="font-bold text-amber-800 ">Meta Description:</span> {g.seoMetaDescription || g.bio}</div>
              <div><span className="font-bold text-amber-800 ">Image Alt Text:</span> {g.imageAltText || `${g.name} Portrait`}</div>
              <div><span className="font-bold text-amber-800 ">Guru Lineage Order:</span> Master #{g.order}</div>
            </div>
          </div>
        )}

        {/* Hero Banner Section */}
        <div className="relative rounded-3xl overflow-hidden bg-gradient-to-br from-amber-50 via-amber-100/50 to-white text-slate-900 shadow-md border border-amber-200">
          <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_top_right,_var(--tw-gradient-stops))] from-amber-200/40 via-transparent to-transparent pointer-events-none" />
          ...
          <div className="relative z-10 p-6 sm:p-10 lg:p-12 flex flex-col md:flex-row items-center gap-8 md:gap-10">
            ...
            <div className="space-y-3 text-center md:text-left">
              <div className="flex flex-wrap items-center justify-center md:justify-start gap-2">
                <span className="px-3 py-1 rounded-full text-xs font-bold bg-amber-500/20 border border-amber-400/40 text-amber-700 uppercase tracking-widest">
                  {isHi ? "गुरु परंपरा आधिकारिक जीवनी" : "Official Guru Biography"}
                </span>
                <span className="px-3 py-1 rounded-full text-xs font-bold bg-white/10 text-slate-800">
                  {g.period}
                </span>
              </div>

              <h1 className="font-serif text-3xl sm:text-4xl lg:text-5xl font-extrabold text-slate-800 leading-tight">
                {isHi && g.sanskritName ? g.sanskritName : g.name}
              </h1>

              {g.sanskritName && !isHi && (
                <p className="text-base sm:text-lg font-serif font-semibold text-amber-700">{g.sanskritName}</p>
              )}

              <p className="text-base sm:text-lg font-medium text-slate-700/90 leading-relaxed max-w-2xl">
                {isHi && g.titleHindi ? g.titleHindi : g.title}
              </p>

              <div className="pt-2 flex flex-wrap items-center justify-center md:justify-start gap-4 text-xs font-semibold text-amber-800">
        ...
        <div className="bg-white rounded-3xl p-6 sm:p-10 border border-amber-200/80 shadow-md space-y-4">
          ...
          <div className="prose prose-amber max-w-none font-serif text-base sm:text-lg leading-relaxed sm:leading-loose text-slate-800 whitespace-pre-line space-y-4">
        ...
        {g.structuredSections && g.structuredSections.length > 0 ? (
          <div className="space-y-8">
            {g.structuredSections.map((sec, idx) => (
              <div 
                key={sec.id || idx}
                className="bg-white rounded-3xl p-6 sm:p-10 border border-amber-200/70 shadow-sm space-y-4 transition-all hover:shadow-md"
              >
                <div className="flex items-center gap-3 border-b border-amber-100 pb-3">
                ...
                <div className="font-serif text-base sm:text-lg leading-relaxed sm:leading-loose text-slate-800 whitespace-pre-line space-y-4">
          ...
          <div className="space-y-8">
            {g.earlyLifeHi && (
              <div className="bg-white rounded-3xl p-6 sm:p-10 border border-amber-200/70 shadow-sm space-y-4">
                <h3 className="font-serif text-xl sm:text-2xl font-bold text-amber-950 flex items-center gap-2">
                  <Flame className="w-5 h-5 text-amber-600" />
                  <span>{isHi ? "बाल्यकाल एवं पारिवारिक पृष्ठभूमि" : "Early Life & Background"}</span>
                </h3>
                <p className="font-serif text-base sm:text-lg leading-relaxed sm:leading-loose text-slate-800 whitespace-pre-line">
        ...
        {((g.lilasList && g.lilasList.length > 0) || (g.lifeMilestones && g.lifeMilestones.length > 0)) && (
          <div className="bg-white rounded-3xl p-6 sm:p-10 border border-amber-200/80 shadow-md space-y-6">
            <div className="flex items-center gap-3 border-b border-amber-100 pb-3">
            ...
            <div className="grid grid-cols-1 gap-6">
              {g.lilasList && g.lilasList.length > 0 ? (
                g.lilasList.map((lila, lIdx) => (
                  <div 
                    key={lIdx} 
                    className="p-5 sm:p-6 rounded-2xl bg-amber-50/50 border border-amber-200/80 space-y-3"
                  >
                    <h3 className="font-serif font-bold text-lg sm:text-xl text-amber-950 flex items-center gap-2">
                    ...
                    <div className="font-serif text-base leading-relaxed sm:leading-loose text-slate-800 whitespace-pre-line pl-0 sm:pl-9">
                ...
                g.lifeMilestones?.map((ms, idx) => (
                  <div 
                    key={idx} 
                    className="p-5 sm:p-6 rounded-2xl bg-amber-50/50 border border-amber-200/80 space-y-2"
                  >
                    <h3 className="font-serif font-bold text-lg text-amber-950 ">
                      {isHi ? ms.titleHi : ms.titleEn}
                    </h3>
                    <p className="font-serif text-base text-slate-800 leading-relaxed whitespace-pre-line">
        ...
        <div className="bg-amber-100/60 rounded-3xl p-6 sm:p-10 border border-amber-300/80 shadow-md space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {(isHi && g.keyTeachingsHindi ? g.keyTeachingsHindi : g.keyTeachings).map((kt, i) => (
              <div key={i} className="flex items-start gap-3 p-4 rounded-2xl bg-white border border-amber-200/80 shadow-sm">
                ...
                <p className="font-serif text-base text-slate-900 leading-relaxed font-medium">
                  {kt}
                </p>
              </div>
            ))}
        ...
        <div className="p-6 sm:p-8 rounded-3xl bg-amber-50 text-slate-800 border border-amber-200 border-l-8 border-l-amber-500 shadow-md space-y-3">
          ...
          <blockquote className="font-serif italic text-xl sm:text-2xl font-semibold text-slate-800 leading-relaxed">
          ...
          {isHi && g.quotesHindiList && g.quotesHindiList.length > 1 && (
            <div className="pt-4 border-t border-amber-700/60 space-y-2">
              <p className="text-xs font-bold text-amber-700 uppercase">{isHi ? "अन्य महत्वपूर्ण अमृत विचार:" : "Other Sacred Sayings:"}</p>
              <ul className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-sm text-slate-700 font-serif">
        ...
        <div className="bg-white rounded-3xl p-6 sm:p-10 border border-amber-200/80 shadow-md space-y-4">
          ...
          <p className="font-serif text-base sm:text-lg leading-relaxed text-slate-800 ">
            {isHi ? 
        ...
        <div className="bg-white rounded-3xl p-6 sm:p-10 border border-amber-200/80 shadow-md space-y-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <div className="rounded-2xl overflow-hidden border border-amber-200/80 bg-amber-50 p-3 text-center space-y-2">
              <div className="h-48 rounded-xl overflow-hidden bg-slate-200 ">
                <img
                  src={g.image}
                  alt={g.name}
                  referrerPolicy="no-referrer"
            ...
            <div className="rounded-2xl overflow-hidden border border-amber-200/80 bg-amber-50 p-3 text-center space-y-2">
              <div className="h-48 rounded-xl overflow-hidden bg-slate-200 flex items-center justify-center p-4">
        ...
        <div className="rounded-3xl bg-amber-100 border border-amber-300 text-slate-800 p-8 sm:p-10 text-center space-y-4 shadow-md">
          ...
          <p className="font-serif text-base sm:text-lg text-slate-700/90 max-w-2xl mx-auto leading-relaxed">
            {isHi ? 
          ...
          <div className="pt-2 flex flex-wrap items-center justify-center gap-4">
            <button 
              onClick={() => setSelectedGuruForBio(null)}
              className="px-6 py-3 rounded-full bg-amber-600 text-white text-sm font-extrabold hover:bg-amber-700 shadow-md transition-all flex items-center gap-2"
            >
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {otherGurus.map((other) => (
              <button
                ...
                className="p-4 rounded-2xl bg-amber-50/50 hover:bg-amber-100/80 :bg-slate-800 border border-amber-200/80 transition-all text-left group flex items-center gap-4"
              >
                <img
                  src={other.image}
                  alt={other.name}
                  ...
                  className="w-16 h-16 rounded-xl object-cover object-top shrink-0 border border-amber-300 shadow-sm"
                />
                <div className="space-y-1 min-w-0">
                  <span className="text-[10px] font-bold text-amber-700 uppercase tracking-widest block">
                    Guru #{other.order}
                  </span>
                  <h3 className="font-serif font-bold text-sm text-slate-900 group-hover:text-amber-800 :text-amber-700 transition-colors truncate">
        ...
        {/* Footer Note */}
        <div className="text-center py-6 border-t border-amber-200/60 text-xs text-slate-500 font-serif space-y-1">
